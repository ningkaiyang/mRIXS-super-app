...
# ===========================================================================
# Reproduction gate (runs only when reference data is available)
# ===========================================================================

class TestReproductionGate:
    """Exact reproduction test against V8 diagnostic reference data.

    Skips if RIXS_sharpness data or reference summary is not available.
    """
    ...
    def reference_data(self):
        ...
        summary_path = os.path.join(
            ...
            "diag7_probe", "summary.json"
        )
        if not os.path.isfile(summary_path):
            pytest.skip("Reference summary.json not found")
        with open(summary_path) as f:
            data = json.load(f)
        return data["grid_results"]["g10_r4_l2_s8"]
    ...
    def data_root(self):
        ...
        if not os.path.isdir(root):
            pytest.skip("RIXS_sharpness data directory not found")
        return root

    def test_all_37_frames(self, reference_data, data_root):
        import glob, os
        import tifffile

        _, config = get_preset(DEFAULT_PRESET_ID)
        scanner = V8RightSideScanner()
        ...
        for sweep_key in sorted(reference_data.keys()):
            sweep_dir_name = sweep_key.replace("_", " ")
            sweep_dir = os.path.join(data_root, sweep_dir_name)
            if not os.path.isdir(sweep_dir):
                continue

            frames = reference_data[sweep_key]["frames"]
            tif_files = sorted(glob.glob(os.path.join(sweep_dir, "*.tif*")))

            for i, ref_frame in enumerate(frames):
                if i >= len(tif_files):
                    continue

                total += 1
                img = tifffile.imread(tif_files[i])
                if img.ndim == 3:
                    img = img.sum(0)
                img = img.astype(np.float32)

                prepared = prepare_frame(img, preproc_config)
                result = scanner.detect(prepared, config)

                exp_cand = ref_frame["candidates"]
                exp_inl = ref_frame["inliers"]
                exp_ang = ref_frame["angle"]

                if result.n_candidates != exp_cand:
                    mismatches.append(
                        f"{sweep_key} f{i:02d}: cand={result.n_candidates} vs {exp_cand}"
                    )
                if result.n_inliers != exp_inl:
                    mismatches.append(
                        f"{sweep_key} f{i:02d}: inl={result.n_inliers} vs {exp_inl}"
                    )
                if result.angle_deg is not None and exp_ang is not None:
                    if abs(result.angle_deg - exp_ang) > 1e-6:
                        mismatches.append(
    ...
    def test_deterministic_repeated_runs(self, reference_data, data_root):
        """Running twice on the same frame must produce identical results."""
        import glob, os
        import tifffile
        ...
        for sweep_key in sorted(reference_data.keys()):
            sweep_dir = os.path.join(data_root, sweep_key.replace("_", " "))
            if not os.path.isdir(sweep_dir):
                continue
            tif_files = sorted(glob.glob(os.path.join(sweep_dir, "*.tif*")))
            if not tif_files:
            ...
            pf1 = prepare_frame(img, preproc_config)
            r1 = scanner.detect(pf1, config)
            pf2 = prepare_frame(img, preproc_config)
            r2 = scanner.detect(pf2, config)

            assert r1.n_candidates == r2.n_candidates
            assert r1.n_inliers == r2.n_inliers
            if r1.angle_deg is not None:
                assert abs(r1.angle_deg - r2.angle_deg) < 1e-10
            return

        pytest.skip("No frames available for determinism test")
